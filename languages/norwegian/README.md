# Cry of Fear: Enhanced - Norwegian (Norsk)

A **minimal language pack**: it makes the menu of Cry of Fear: Enhanced
Norwegian and selects the game's own Norwegian subtitles and documents.
It adds no game text of its own.

* `manifest.txt` - `display_name=Norsk`, `codepage=1252`,
  `subtitle_language=5`: picking *Norsk* in Options > Game >
  Language sets the engine's `cof_language norwegian` and the client's own
  subtitle slot `cof_subtitlelanguage 5`, so the game reads the Norwegian
  files it ships (`txtfiles/languages/norwegian/`, `notes/languages/norwegian/`,
  `gfx/vgui/introductions/norwegian/`).
...
-Pack languages\norwegian` (missing strings, `%s` mismatches, characters
