# Cry of Fear: Enhanced - German (Deutsch)

A **minimal language pack**: it makes the menu of Cry of Fear: Enhanced
German and selects the game's own German subtitles and documents.
It adds no game text of its own.

* `manifest.txt` - `display_name=Deutsch`, `codepage=1252`,
  `subtitle_language=4`: picking *Deutsch* in Options > Game >
  Language sets the engine's `cof_language german` and the client's own
  subtitle slot `cof_subtitlelanguage 4`, so the game reads the German
  files it ships (`txtfiles/languages/german/`, `notes/languages/german/`,
  `gfx/vgui/introductions/german/`).
...
-Pack languages\german` (missing strings, `%s` mismatches, characters
