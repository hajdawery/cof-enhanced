# Cry of Fear: Enhanced - French (Français)

A **minimal language pack**: it makes the menu of Cry of Fear: Enhanced
French and selects the game's own French subtitles and documents.
It adds no game text of its own.

* `manifest.txt` - `display_name=Français`, `codepage=1252`,
  `subtitle_language=3`: picking *Français* in Options > Game >
  Language sets the engine's `cof_language french` and the client's own
  subtitle slot `cof_subtitlelanguage 3`, so the game reads the French
  files it ships (`txtfiles/languages/french/`, `notes/languages/french/`,
  `gfx/vgui/introductions/french/`).
...
-Pack languages\french` (missing strings, `%s` mismatches, characters
