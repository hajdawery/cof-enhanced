# Cry of Fear: Enhanced - Dutch (Nederlands)

A **minimal language pack**: it makes the menu of Cry of Fear: Enhanced
Dutch and selects the game's own Dutch subtitles and documents.
It adds no game text of its own.

* `manifest.txt` - `display_name=Nederlands`, `codepage=1252`,
  `subtitle_language=2`: picking *Nederlands* in Options > Game >
  Language sets the engine's `cof_language dutch` and the client's own
  subtitle slot `cof_subtitlelanguage 2`, so the game reads the Dutch
  files it ships (`txtfiles/languages/dutch/`, `notes/languages/dutch/`,
  `gfx/vgui/introductions/dutch/`).
...
-Pack languages\dutch` (missing strings, `%s` mismatches, characters
