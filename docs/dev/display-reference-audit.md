...
> **Status (2026-09-22 docs pass):** written before the unified UI. The visible
> main menu is now drawn by the engine menu (MainUI with the Cry of Fear theme,
> [UI theme](../design/ui-theme.md)), no longer by the client's `GameMenu.res`
> path, and the in-game UI scales through `cof_ui_scale`
> ([UI scaling](../design/ui-scaling.md)). The acceptance matrix below is kept as
> the reference for resolution checks.
