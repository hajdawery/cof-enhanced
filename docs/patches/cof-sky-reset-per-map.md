...
## Round 2, milestone 5b: every level start, and no `desert` at all

User report, 2026-09-22 evening: "sometimes the sky is not re-enabled when
changing maps". The first round restored `gl_customsky`, so the cvar read `1`
again - and the sky still did not draw. **Measured** (client disassembly, same
image base):
...
every attempt. A failed load (a map with no `skyname`, so `desert`) leaves
`loaded_flag` 0 with `cached_name` still `black`; the next map asking for
`black` takes the early return and never sets `loaded_flag` again, so the draw
pass (`100637C7`) returns on `!loaded_flag`: `gl_customsky` reads `1`, the
console is silent, and the sky is missing. 162 of the 235 maps use `black`, 56
have no `skyname`, so `black -> no skyname -> black` is ordinary play.

The fix, three parts (`patches/cof-sky-reset-per-map.patch`, regenerated in
place; the apply script gained the three server files):

* `CL_CoF_SkyResetPerMap` runs at **every** level start - from `R_SetupSky`,
  which `CL_RegisterResources` reaches on every level load, and from
  `CL_ClearState` (disconnect, `map`, save load) - and the pending restore
  survives any number of level starts until it is carried out;
* it only ever restores a `0` the engine watched the **client** write
  (`CL_CoF_SkyClientCvarSet`); a user's own `gl_customsky 0` is never
  overridden;
* the real cure, server side: `cof_default_skyname` (archived, default
  `black`) replaces the stock `sv_skyname` default `desert` for Cry of Fear
  (`SV_CoF_ApplyDefaultSkyName`, called right after `Cvar_Reset( "sv_skyname" )`
  in `SV_SpawnEntities`, so a map that does carry a `skyname` still wins). The
  client's loader then succeeds on all 56 skyname-less maps and the trap above
  is never armed. `cof_default_skyname ""` is stock behaviour.

### Verification (m5b)

Fixture `stage1/m5b-integration-20260922/fixture`, windowed 1280x720,
`+volume 0`, every command from `maps/<map>_load.cfg`
(`case-sky.ps1`): `c_bridge` (`black`) -> `c_nightmare2` (no skyname) ->
`c_carnie1` (no skyname) -> `c_forest1` (`black`), each started with `map`,
each printing `gl_customsky`, `sv_skyname`, `cof_default_skyname` and taking a
screenshot. Engine `BAF1533FC39A67D7CE86CB1F269EA1D71B3927081C730B1F68C3CDE1E90B9D5C`.

| Run | settings | Decisive lines |
| --- | --- | --- |
| `sky2-stock` | `cof_default_skyname ""`, `cof_sky_reset_per_map 0` | `Sky error: cant load sky face gfx/env/desertlf.tga` on `c_nightmare2`, then `gl_customsky 0` on every later map; **white sky** in `sky2-stock-c_forest1.png` |
| `sky2-m5a` | `cof_default_skyname ""`, reset 1 (the first round) | `Sky error` on both skyname-less maps, `[cof-sky] restored "gl_customsky" to "1"` each time, `gl_customsky 1` on `c_forest1` - and still a **white sky** there (`sky2-m5a-c_forest1.png`): the cached-name trap |
| `sky2-fix`, `rel-sky` | defaults | `sv_skyname black` and `gl_customsky 1` on all four maps, **no `Sky error` line**, black sky on `c_forest1` |
| `sky2-user0` | defaults + `gl_customsky 0` typed on `c_bridge` | `gl_customsky 0` on all four maps: the user's choice is kept |
| `sky2-user0-nodefault` | `cof_default_skyname ""` + `gl_customsky 0` | the same, and no restore: the pass is off, the loader never runs |

`c_forest1` does have sky in the spawn view, so unlike the first round the
white sky is visible in the screenshots: `sky2-stock-c_forest1.png` and
`sky2-m5a-c_forest1.png` show it, `sky2-fix-c_forest1.png` does not.

Patch `patches/cof-sky-reset-per-map.patch` 18 352 bytes, SHA-256
`5C555365887ADB89178CC54049F6AEAC15D42139A1C21022675594420572CB00`, identical
in effect to the first-round patch plus `stage1/sky2-20260922/apply-sky2-delta.py`
(checked file by file); round-tripped where it sits in the stack by
`stage1/m5b-integration-20260922/stackverify-m5b.ps1`.
