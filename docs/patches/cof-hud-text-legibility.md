...
[the in-game UI scaled from the display](../design/ui-scaling.md) and the VGUI font
half is [the client's VGUI text rasterised from Inter](../design/vgui-inter-fonts.md);
...
## 11. No strip under the hint bar, milestone 5b

User report after m5a: the backing strip under the top hint bar ("Press E to
...", "Using Gas Mask") looks wrong - that bar is CHUDControl's own Label with
its own painted background, so the strip drew a second, darker box inside it.

`XashSurface::runIsTopBand()` (`surface.cpp`, declared in `support.h`) asks the
same question as `runIsMessage()` - a short run of a retargeted role, measured
at its own device row BEFORE any move - and answers true when that row is above
the message band (`MSG_BAND_LOW_PCT`). The strip condition in
`flushBackingText()` becomes

| run | strip | face |
| --- | --- | --- |
| pager (`textYTarget < 0`), notes and documents | yes | SemiBold |
| message band, 3 lines or fewer (pickups, subtitles) | yes | SemiBold |
| **top band**, 3 lines or fewer (the hint bar) | **no** | SemiBold |
| more than 3 lines (intro card, credits roll) | no | as before |

A `[cof-vguirect]` / `[cof-vguitex]` line per filled / textured VGUI rectangle
joins the `cof_hud_text_trace` log (it is how the chapter card's rule was shown
NOT to be a VGUI draw; see `docs/patches/cof-ads-toggle.md`).

Measured (`stage1/m5b-integration-20260922`, `case-band.ps1`, `c_forest3`,
1920x1080, notify lines off): `cof_font_probe "credits TOP BAND HINT STANDIN"`
at (12,12) plus `cof_hud_msg_probe`, once with the m5a `vgui.dll`
(`band2-m5avgui-b-both.png`: a dark box behind the top-band text) and once with
the m5b one (`band2-m5b-b-both.png`, `rel-band-b-both.png`: no box, same
SemiBold face; the message-band run keeps its strip in both).
`patches/cof-hud-text-backing.patch` was regenerated in place (66 610 bytes,
SHA-256 `A59DFB7E2B407170FC49CA6C56B5BF8548D53D3126E4871E4780B9135D3AA3CB`);
`vgui.dll` `34A5CF6E112891AB8FB0074ED68388BF7FF28486FFB26B7789F34B1807E536B7`.
