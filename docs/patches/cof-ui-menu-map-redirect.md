...
| `d0-baseline`, `d1-baseline-trace` (2026-09-22) | a real death with `cl_trace_stufftext 1` and `cl_trace_messages 1`: no stufftext at all, and the only relevant message is `VGUIMenu` index 35. See `docs/patches/cof-ui-death-flow.md` |
...
* The `hl.dll` stufftext producer was **not observed firing at runtime**. Its
  ...
  **Death is no longer one of those paths, measured.** `docs/patches/cof-ui-death-flow.md`
...
helpers (that order now lives in [the patch stack](../dev/patch-stack.md)) up to and including `apply-cof-vid-restart-background.ps1`, the patch
