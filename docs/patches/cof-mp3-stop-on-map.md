...
| --- | --- | --- | --- |
| `cof_mp3_stop_on_map` | `1` | none (deliberately not `FCVAR_ARCHIVE`, like the other engine-fix `cof_*` switches; the player options such as `cof_fov` or `cof_language` are archived) | run `stopmp3` before a fresh map, newgame, save load or disconnect |
