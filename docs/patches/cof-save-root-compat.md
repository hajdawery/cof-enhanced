...
> **Status (2026-09-22 docs pass):** the cvar still defaults to `0` in the
> engine, but the project launcher `CoFLaunchApp.exe` always injects
> `+set cof_save_root_compat 1` ([launcher contract](../dev/steam-launch-prototype.md#launcher-contract)),
> so it is on in every normal Cry of Fear: Enhanced run. The "not enabled by default"
> sentence at the end refers to the engine default only.
