...
> **Reading guide.** This page grew round by round. Later sections supersede
> earlier ones where they disagree: the main list, the Options tree, the
> Language page and the pause list were all reshaped after milestone 3a (see
> "Options audit, main-list restructure ...", "Milestone 5b" and
> [language packs](language-packs.md) section 7).
...
the build machine. *(Corrected 2026-09-22: they are not game data. The three
TTFs are unmodified Inter 3.019, open-licensed font software, and **are**
committed under `gamedata/cryoffear/gfx/fonts/` with `OFL.txt`; see
[LICENSING.md](../../LICENSING.md) and [fonts](fonts.md).)*
...
[the death flow and "Enable console"](../patches/cof-ui-death-flow.md):
...
as the `d*` and `b*` runs listed in `docs/patches/cof-ui-death-flow.md`.
...
`docs/patches/cof-ui-death-flow.md` §1.1 — and started `game_over.mp3` by the time
...
> **Translations (lang2, 2026-09-22).** `L()` still only resolves tokens to
> English. The menu's other languages come from the active language pack's
> `strings/menu-strings.tsv`, applied where text is drawn and measured
> (`UI_LangText` next to `L()`, `patches/cof-mainui-menu-strings.patch`), so
> a change of `cof_language` switches pages that already resolved their `L()`
> strings; see `docs/design/language-packs.md` section 8.
...
> **TODO (licensing, 2026-09-22):** add an attribution line to this page at the
> next regeneration of the theme patch: "Cry of Fear: Enhanced runs on Xash3D
> FWGS (GPL), FreeVGUI (BSD) and Inter (OFL)." Not required by the licences
> (upstream shows no legal-notice screen), but recommended; see
> [`LICENSING.md`](../../LICENSING.md) section 4 and
...
[its own document](../patches/cof-ui-sound-volume.md) and its own patch
...
| Command | Runs |
| --- | --- |
| `menu_cof_quit_to_menu` | `CMenuMain::DisconnectCb`, i.e. Quit to menu plus its confirmation, for the disconnect-return validation in [the redirect document](../patches/cof-ui-menu-map-redirect.md#disconnect-re-arms-the-background-map-too-2026-09-22) |
...
`stage1/fov-impl-20260922/RESULTS.md`, `docs/patches/cof-fov.md`) adds two archived
...
## Milestone 5b: pause list, aim option, default binds, unlockables, 2026-09-22

Evidence `stage1/m5b-integration-20260922` (drivers `case-pause.ps1`,
`case-ads2.ps1`, `case-unlock.ps1`); menu `E3E437A4903CD7DBE920E21FE8D42A934D565D5D71D36BD60ACC6A51087DDC77`
(`stage1/releases/m5b-20260922`); `patches/cof-mainui-source-theme.patch`
regenerated in place (250 155 bytes, SHA-256
`456CD0271EB4F1B2F8C662F6E388DDDE5EE15FE69EF5444FDDE3C139BE43913B`).

### Pause list: Save Game and Load Game

`menus/Main.cpp`: the in-game list is Resume game, **Save Game**, **Load Game**,
Options, Quit to menu, Quit. Save Game (`cofSaveGame`, `UI_SaveGame_Menu`) is
on the list only while `cof_pause_menu_saves` is 1; Load Game is `saveRestore`
with `UI_LoadGame_Menu`. Both open the shared save/load page with
`SetSaveMode()` before `Show()` (the m5a fix in `menus/LoadGame.cpp`).
`CMenuMain::Think()` lays the list out again when the option is switched on the
Game page while the pause list is underneath it.
Measured (`pause1`, `rel-pause`): with the cvar 0 the list has no Save Game;
set to 1 while the list is open, Save Game appears between Resume and Load;
`menu_savegame` / `menu_loadgame` open the page titled SAVE GAME with a Save
button and LOAD GAME with a Load button, both inside the panel. Clicking the
two items is a manual check (no input injection); they call the same functions.

### "Aim down sights" on the Game page

`menus/AdvancedControls.cpp`: a spinner Hold / Toggle bound to the engine's
archived `cof_ads_toggle`, written live; the engine keeps the game's own
`ironsights_toggle` on it (`docs/patches/cof-ads-toggle.md`). New row under Subtitle
language / HUD scale, left column; the right column is left free.

**m6:** the right-hand slot now holds the **Language** spinner (the engine's
language packs, `cof_language`) with a one-line restart note under the row, and
the panel is 760x580 for Cry of Fear. It is its own patch,
...
`+attack2` / `+attack3` (`docs/patches/cof-ads-toggle.md` section 3), with
`gfx/shell/kb_def.lst` in gamedata for "Use defaults".

### Unlockables: the seven unidentified flags are not unlockables

The user's all-unlocked reference `scriptsettings.dat` (SHA-256
`7C76FEC82FD546BB8DA6FFFF863EE464E14801B7EED0AC645A3B734D947579AA`) differs
from the canonical file in exactly the 27 named lines (3, 39, 51-62, 70-80, 82,
83). The seven "Unidentified flag (line N)" pairs (7, 15, 22, 45, 47, 48, 81)
keep their token in a fully unlocked save, so they are flagged `hidden` in
`s_cofUnlockables` (still answered by `UI_CoFUnlockableStatus`) and the page
lists 27 rows. A developer line on every read gives the whole page in the log.
Measured with the fixture's `scriptsettings.dat` replaced by a copy of the
reference file and restored afterwards (`unlock2-all`, `unlock2-canon`):
